...
export function LegacyTransactionsPage() {
...
export function TransactionsPage() {
  ...
  const [draft, setDraft] = useState({ search: initialSearch, status: '' })
  const [filters, setFilters] = useState(draft)
  const [page, setPage] = useState(1)
  const [selected, setSelected] = useState(null)
  const dates = view === 'today' ? { from: today, to: today } : { to: yesterday }
  const resource = useData(`/reports/transactions?${queryString({ ...filters, ...dates, page })}`)
  const result = resource.data?.[0]
  const changeView = (next) => { setView(next); setPage(1) }

  return <><PageHeading eyebrow="Payment records" title={view === 'today' ? 'Transactions today' : 'Transaction history'} description={view === 'today' ? 'Transactions opened today (UTC).' : 'Transactions from yesterday and earlier.'} /><Tabs values={[["today", 'Today'], ["history", 'Transaction history']]} value={view} onChange={changeView} /><form className="a-filters" onSubmit={(event) => { event.preventDefault(); setPage(1); setFilters({ ...draft }) }}><Field label="Search transactions" placeholder="Order, table, or transaction ID" maxLength={100} value={draft.search} onChange={(event) => setDraft({ ...draft, search: event.target.value })} /><Field label="Status"><select value={draft.status} onChange={(event) => setDraft({ ...draft, status: event.target.value })}><option value="">All statuses</option><option value="open">Open</option><option value="partially_paid">Partially paid</option><option value="paid">Paid</option><option value="cancelled">Cancelled</option></select></Field><Button icon="search">Apply</Button></form><DataState resource={resource}><Panel title={view === 'today' ? 'Today’s transactions' : 'Previous transactions'} className="a-table-panel"><Table headings={['Transaction', 'Order', 'Table', 'Amount', 'Method', 'Status', 'Date']} rows={(result?.data || []).map((row) => <tr key={row.id}><td><button className="a-text-button" onClick={() => setSelected(row)}>{row.transaction_number}</button></td><td>{row.order_numbers?.join(', ') || '—'}</td><td>Table {row.table?.table_number || '—'}</td><td><strong>{currency(row.total_amount)}</strong></td><td>{row.payment_methods?.map((method) => method === 'gcash' ? 'GCash' : 'Cash').join(' / ') || '—'}</td><td><Badge value={row.status} /></td><td>{dateTime(row.opened_at)}</td></tr>)} empty={view === 'today' ? 'No transactions have been opened today.' : 'No previous transactions found.'} /><Pagination meta={result?.meta} onPage={setPage} /></Panel></DataState>{selected && <Modal title={selected.transaction_number} onClose={() => setSelected(null)}><dl className="a-summary-list"><div><dt>Status</dt><dd><Badge value={selected.status} /></dd></div><div><dt>Total</dt><dd>{currency(selected.total_amount)}</dd></div><div><dt>Paid</dt><dd>{currency(selected.paid_amount)}</dd></div><div><dt>Remaining balance</dt><dd>{currency(selected.remaining_balance)}</dd></div><div><dt>Receipt</dt><dd>{selected.receipt?.receipt_number || 'Not issued'}</dd></div></dl></Modal>}</>
}
